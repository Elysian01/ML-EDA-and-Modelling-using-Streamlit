...
def modelling(df):
    ...
    test_size = st.slider('Select the size of Test Dataset', min_value=0.0, max_value=1.0, value=0.3, help='Set the ratio for splitting the Train and Test Dataset')

    #Set the size of K (only for K-NN)
    if selected_algo=="K_Nearest_Neighbors_Classifier":
        k_size = st.number_input('Select the size of K', min_value=0, max_value=100, value=0, step=1, help='Set the ratio for splitting the Train and Test Dataset')
